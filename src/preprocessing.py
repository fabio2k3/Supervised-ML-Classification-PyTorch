"""
Preprocessing and Feature Engineering Module for the House Prices dataset
"""
...
class HousePricePreprocessor:
    """
    Complete preprocessor for the House Prices dataset
    Includes: cleaning, imputation, feature engineering, encoding, and scaling
    """
    ...
    def handle_missing_values(self, df):
        ...
        # Variables where NA means "None" or "Does not exist"
        none_vars = [
        ...
        # Numerical variables related to garage / basement
        zero_vars = [
        ...
        # LotFrontage: impute using neighborhood median
        if 'LotFrontage' in df.columns:
            df['LotFrontage'] = df.groupby('Neighborhood')['LotFrontage'].transform(
                lambda x: x.fillna(x.median())
            )
        
        # Remaining categorical variables: mode
        categorical_cols = df.select_dtypes(include=['object']).columns
        for col in categorical_cols:
            if df[col].isnull().sum() > 0:
                df[col].fillna(df[col].mode()[0], inplace=True)
        
        # Remaining numerical variables: median
    ...
    def remove_outliers(self, df, target_col='SalePrice'):
        """Remove extreme outliers based on dataset analysis"""
        ...
        # Known outliers from the House Prices dataset
        df = df.drop(df[(df['GrLivArea'] > 4000) & (df[target_col] < 300000)].index)
        df = df.drop(df[(df['TotalBsmtSF'] > 6000)].index)
        
        print(f"✅ Outliers removed: {initial_rows - df.shape[0]} rows")
        return df
    ...
    def create_features(self, df):
        ...
        # 9. Basement indicator
        df['HasBsmt'] = (df['TotalBsmtSF'] > 0).astype(int)
        
        # 10. Second floor indicator
    ...
    def encode_categorical(self, df, target_col='SalePrice'):
        ...
        # Ordinal variables with explicit ordering
        ordinal_mappings = {
    ...
    def inverse_transform_target(self, y_log):
        """Inverse log transformation of the target"""
    ...
    def preprocess_pipeline(self, train_df, test_df=None, target_col='SalePrice'):
        ...
        Returns:
            X_train, y_train, X_test (if exists), test_ids
        """
        print("\n" + "="*60)
        ...
        test_ids = (
            test_df['Id'] if test_df is not None and 'Id' in test_df.columns else None
        )
        ...
        # 1. Remove outliers (train only)
        if y_train is not None:
            combined = pd.concat(
                [train_df, pd.DataFrame({target_col: y_train})],
                axis=1
            )
        ...
        # 2. Concatenate train and test for consistent processing
        if test_df is not None:
        ...
        if y_train is not None:
            ...
            print("✅ Target transformed: log(SalePrice)")
        else:
            y_train_log = None
